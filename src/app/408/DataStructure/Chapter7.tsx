  ...
  return (
    <div className="prose daisy-prose p-6 max-w-[1300px]">
      ...
      <p>
        ...
        其中, <Latex>{`当$P_j=\\frac{1}{n}$时, $ASL_\\text{表中元素}=\\frac{n+1}{2},~ASL_\\text{不在表中元素}=n+1$.`}</Latex>
        <div className="h-3" />
        有序线性表的顺序查找: <br />
        ...
        可知, <Latex>{`$ASL_\\text{不在表中元素}=\\frac{1+2+\\cdots+n+n}{n+1}=\\frac{n}{2}+\\frac{n}{n+1}$`}</Latex>. (假设查找元素在各区间节点概率相同)
      </p>
      <h3>二分查找</h3>
      <p>
      ...
      <p>
        <div className="text-xl font-bold">哈希表</div> 
        <span>基本概念: </span> <br />
        哈希函数: 将关键字映射为该关键字对应地址的函数(Hash(key)=Addr). <br />
        冲突: 哈希函数把两个或两个以上不同关键字映射到同一地址. <br />
        哈希表: 根据关键字直接进行访问的数据结构, 建立了关键字和地址间的直接映射关系. <br />
        <blockquote className="mt-1 mb-1">
          哈希函数定义域必须包含全部关键字, 值域范围依赖于哈希表的大小. <br />
          哈希函数映射到的地址应尽可能均匀分布在整个哈希表的地址空间, 从而减少聚集现象减少冲突 <br />
          哈希函数应尽可能简单, 能较短时间计算出任一关键字对应的地址. <br />
        </blockquote>
        <div className="h-3"/>
        常用哈希函数: <br />
        直接定址法: <Latex>{`$H(key)=key~$或$~H(key)=a\\times key + b$`}</Latex> (适合关键字分布基本连续的情况, 否则空位较多造成空浪费) <br />
        除留余数法: <Latex>{`$H(key)=key ~\\%~ p$`}</Latex> (<Latex>{`设散列表表长为$m$, 取一个不大于$m$但最接近或等于$m$的质数$p$, 使得关键字等概率(均匀)映射到哈希表的所有地址, 从而尽可能减少冲突`}</Latex>)
        <blockquote className="mt-1 mb-1">为什么<Latex>{`$p$`}</Latex>要取质数: 因为质数没有除了1以外的因数, 可避免因数导致的周期性模式. 比如说, 令p=6则3的倍数都只会映射到0和3的位置, 而不是均匀分布. </blockquote>
        数字分析法: 设关键字是r进制数(如十进制数), 而r个数码在各位上出现的频率不一定相同, 可能在某些位上分布均匀一些, 而在其他位上分布不均匀, 此时应选取数码分布较均匀的若干位作为哈希地址. 这种方法适合于已知的关键字集合, 若变更关键字集合, 则需重新构造哈希函数. <br />
        平方取中法: 取关键字平方值的中间几位作为哈希地址, 具体取多少位要视实际情况而定. 这种方法映射到的哈希地址与关键字的每位都有关系，因此哈希地址分布比较均匀, 适用于关键字的每位取值都不够均匀或关键字位数小于哈希地址所需位数的情况. <br />
        <blockquote className="mt-1 mb-1">不同的情况下, 不同的哈希函数具有不同的性能, 采用何种构造哈希函数的方法取决于关键字集合的情况. </blockquote>
        <div className="h-3"/>
        处理冲突的方法: <br />
        (任何哈希函数都不可能绝对避免冲突, 冲突时要为冲突关键字寻找下一个可用Hash地址, 用<Latex>{`$~H_j~$`}</Latex>表示处理冲突时第<Latex>{`$~j~$`}</Latex>次探测的哈希地址, 若第<Latex>{`$~k~$`}</Latex>次探测没有冲突, 则<Latex>{`$~H_k~$`}</Latex>为关键字在哈希表中的最终地址) <br />
        1. 开放定址法: <br />
        <Latex>{`$H_j=(Hash(key)+d_j)\\%m$`}</Latex>, 其中<Latex>{`$~d_j~$`}</Latex>为增量序列, <Latex>{`$m~$`}</Latex>为哈希表表长. <br />
        增量序列通常有以下几种取法: <br />
        <div className="pl-[1rem]">
          ① 线性探测法: <Latex>{`$d_j=1, 2,\\cdots, m-1$`}</Latex>. (容易造成大量元素在相邻哈希地址聚集, 降低查找效率)<br />
          ② 平方探测法(一般指双向二次探测法): <Latex>{`$d_j=1^2, -1^2, 2^2, \\cdots, k^2, -k^2$`}</Latex>, 其中 <Latex>{`$k<=m/2, ~m$是可以表示为4k+3的质数. (可以证明此时搜索链的前$m$项$~H_1, H_2, \\cdots, H_m~$互异)`}</Latex><br />
          ③ 双散列法: <Latex>{`$d_j=j\\times Hash_2(key), ~j$初始为$0$`}</Latex>. <br />
          ④ 随机序列法: <Latex>{`$d_j=random$()`}</Latex>. <br />
        </div>
        <blockquote className="mt-1 mb-1">注意: 使用开放定址法时, 不能直接物理删除表中已有元素, 否则会截断同义词元素的查找路径. 因此删除一个元素是只能进行逻辑删除(作删除标记), 所以进行多次删除后, 哈希表利用率变低. </blockquote>
        2. 拉链法: <br />
        所有同义词存储在一个线性链表中, 此线性链表由其散列地址唯一标识. <br />
        <div className="h-3"/>
        <blockquote className="mt-1 mb-1">
          平均查找长度仍是衡量哈希表查找效率的度量, 哈希表平均查找长度直接依赖于装填因子<Latex>{`$~\\alpha=\\frac{\\text{表中记录数n}}{\\text{哈希表长度m}}$(不直接依赖于$~m~$或$~n$)`}</Latex> <br />
          此外平均查找长度也与哈希函数的选择(影响同义词数量多少), 冲突解决策略有关(聚集导致非同义词冲突的平均查找(探测)长度更长). <br />
        </blockquote>
       </p>
    </div>
  )
