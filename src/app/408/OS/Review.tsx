    ...
    <div className="prose daisy-prose p-6 max-w-[1500px]">
      ...
      <p>
        ...
        <div className="pl-[1rem] ml-[2px] border-l-[3px] border-amber-200">
          ...
          装入后符号地址才是内存地址(如果在分页系统中则为逻辑地址, 因为分页系统中指令使用逻辑地址); 特点是装入程序时必须分配其要求的全部物理内存空间, 装入时一次性将所有符号逻辑地址重定位到内存地址. <br />
          <strong>动态运行时装入(动态重定位): </strong>
          动态运行时装入(动态重定位): 程序在实际运行时才确定符号的完整内存地址, 即编译后的文件中符号的地址和装入内存后符号的地址都为其段内地址(对未装入模块的符号引用暂时不管), 
