    ...
    <div className="prose daisy-prose p-6 max-w-[1500px]">
      ...
      <p>
        ...
        <div className="pl-[1rem] ml-[2px] border-l-[3px] border-amber-200">
          ...
          <strong>可重定位装入(静态重定位): </strong>程序在装入时确定符号的内存地址, 即编译后的文件中符号的地址仍为符号在对应代码段/数据段中的相对偏移, 装入后符号地址才是内存地址(如果在分页系统中则为逻辑地址, 因为分页系统中指令使用逻辑地址); 
          特点是装入程序时必须分配其要求的全部物理内存空间, 装入时一次性将所有符号逻辑地址重定位到内存地址. (多用于早期多道批处理系统) <br />
          <strong>动态运行时装入(动态重定位): </strong>
          动态运行时装入(动态重定位): 程序在实际运行时才确定符号的完整内存地址, 即编译后的文件中符号的地址和装入内存后符号的地址都为其段内地址(在对应代码段/数据段中的相对偏移量), 
