  ...
  return (
    <div>
        ...
        <ul className="m-0">
          ...
          <li>
            ...
            <ul>
              ...
              <li><strong>是否可抢占? </strong><strong>非抢占式算法</strong>. 因此只有当前运行的作业/进程主动放弃处理机时，才需要调度，计算响应比. </li>
          ...
          <blockquote className="m-0 mb-2 mt-[-0.5rem]">
            FCFS考虑了等待时间(先加入就绪队列的进程比后加入就绪队列的进程等待时间更长), 未考虑运行时间; SJF考虑了运行时间, 未考虑等待时间; HRRN综合考虑了等待时间和运行时间(响应比). <br />
            SJF和SRTN都可能导致饥饿, 是否会导致饥饿与是否为抢占式算法无关, 而是与队列能否插入有关.
          </blockquote>
        </ul>
      </details>
      <details open>
          ...
          <li><strong>优先级调度 (Priority Scheduling)</strong>
            <ul>
              <li>
                  ...
                  实时操作系统中作业限时完成, 应该可以不依赖时间片; 也可以设置多个优先级队列集合时间片轮转调度.
              </li>
              <li>
            ...
            <ul>
              ...
              <li>
                  ...
                  <ol>
                      ...
                      <li>新进程到达时先进入第1级队列, 按FCFS原则排队等待分配时间片。若用完时间片进程还未结束，则进程进入下一级队列队尾. </li>
              ...
              <li>
                  <strong>是否可抢占:</strong> 是抢占式的算法. 在k级队列的进程运行过程中, 若更上级的队列(1~k-1级)中进入了一个新进程, 则新进程会抢占处理机. 原来运行的进程放回第k级队列队尾(重置时间片).
              </li>
              <li>
                  <strong>优点: </strong>
                  对各类型进程相对公平(FCFS的优点); 每个新到达的进程都可以很快就得到响应(RR的优点); 短进程只用较少的时间就可完成(SPF的优点), 而且不必估计进程的运行时间; 
      ...
      <blockquote className="m-0 mb-2">
        多道批处理系统非抢占式调度算法多, 交互式系统抢占式调度算法多. <br />
      </blockquote>
      <blockquote className="m-0 mb-2">
        <strong>多个流程完全相同的任务, 使其流水线执行即可让总时间最短. (可证明)</strong> <br />
      </blockquote>
    </div>
  )
}
