    ...
    <div className="prose daisy-prose max-w-[1500px] leading-[35px] p-6">
      ...
      <blockquote className="whitespace-pre-line w-[1500px]">
        {` docker run -d \\
          ...
          gitea/gitea:latest
        `}
        注意: 1. 要将外部目录中的配置文件"app.ini"挂载到内部目录: /data/gitea/conf (或者将所有相关文件整体挂载到内部目录: /data) <br /> 
        <span className="pl-[3rem]"/> 2. network host 选项可以让容器使用宿主机的网络环境, 然后可以 -e GITEA__SERVER__HTTP_PORT=3002 从而用宿主机的端口访问 Gitea.  <br /> 
        <span className="pl-[3rem]"/> 3. "app.ini"中要添加以下配置才能在页面中直接迁移本地仓库: <br />
        <span className="pl-[5rem]"/> [security]  <br /> 
        <span className="pl-[5rem]"/> IMPORT_LOCAL_PATHS = true  <br /> 
        <span className="pl-[3rem]"/> 同时, 要将仓库在宿主机的目录挂载到内部目录, 之后迁移使用的"本地路径"是内部目录中的仓库路径.  <br /> 
        <span className="pl-[3rem]"/> (但迁移lfs好像有问题, 暂时不推荐)  <br /> 
        <span className="pl-[3rem]"/> <strong>推荐直接在gitea中创建一个新的空仓库, 该仓库实际上为裸仓库(存放在内部目录"/data/gitea/data/gitea-repositories/用户名/仓库名.git"), <br /> 
        <span className="pl-[3rem]"/> 然后从本地仓库push. 用这种方法能把lfs大文件自动push到内部目录的"/data/gitea/data/lfs". <br /> 
        <span className="pl-[3rem]"/> 为避免每次push都要输入用户名密码, 可以启用保存用户名密码: git config --global credential.helper store </strong> <br /> 
      </blockquote>
    </div>
  )
}